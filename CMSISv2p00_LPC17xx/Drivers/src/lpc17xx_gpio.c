...
FunctionalState GPIO_GetIntStatus(uint8_t portNum, uint32_t pinNum, uint8_t edgeState)
{
    if((portNum == 0) && (edgeState == 0))//Rising Edge
        return (FunctionalState)((LPC_GPIOINT->IO0IntStatR) & (1<<pinNum));
    else if ((portNum == 2) && (edgeState == 0))
        return (FunctionalState)((LPC_GPIOINT->IO2IntStatR) & (1<<pinNum));
    else if ((portNum == 0) && (edgeState == 1))//Falling Edge
        return (FunctionalState)((LPC_GPIOINT->IO0IntStatF) & (1<<pinNum));
    else if ((portNum == 2) && (edgeState == 1))
        return (FunctionalState)((LPC_GPIOINT->IO2IntStatF) & (1<<pinNum));
    else
        //Error
        while(1);
}
